...
class Kardex {
    ...
    async StepOne() {
        ...
        const url = common_js_1.Urls.SIIAU_KARDEX_S1
            .replace('?1', this.Session.StudentBasics.pid)
            .replace('?2', this.Session.StudentBasics.carrera);
        const req = await (0, downloader_js_1.Fetch)(url, {}, this.Session.Jar);
    ...
    async StepTwo(formData) {
        const req = await (0, downloader_js_1.Fetch)(common_js_1.Urls.SIIAU_KARDEX_S2, {
            body: formData,
            method: "POST"
        }, this.Session.Jar);
        const $inputs = req.$.querySelectorAll('input[type="hidden"]');
    ...
    async GetData(formData) {
        const req = await (0, downloader_js_1.Fetch)(common_js_1.Urls.SIIAU_KARDEX_SFINAL, {
            method: "POST",
            body: formData
        }, this.Session.Jar);
        return req.$;
    ...
    async Student($) {
        if (!$) {
            $ = await this.GetData(await this.StepTwo.bind(this)(await this.StepOne.bind(this)()));
        }
    ...
    async Kardex() {
        if (!this.Session.Oppened)
            throw new SessionError_js_1.SessionError(SessionError_js_1.SessionErrors.NotLogged);
        const $ = await this.GetData(await this.StepTwo.bind(this)(await this.StepOne.bind(this)()));
